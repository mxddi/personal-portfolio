...
export function ProjectListRow({ project }: { project: Project }) {
  ...
  return (
    <Link
      href={href}
      {...externalProps}
      className="group grid grid-cols-1 items-start gap-3 py-5 transition-colors duration-200 hover:bg-zinc-50 sm:grid-cols-12 sm:items-center sm:gap-4 sm:px-4"
    >
      <div className="flex items-center gap-2 font-mono text-sm text-zinc-600 sm:col-span-2">
        <time dateTime={project.date}>{formatDate(project.date)}</time>
      </div>

      <div className="flex flex-col gap-1 sm:col-span-4">
        <h3 className="flex flex-wrap items-center gap-2 text-base font-medium text-zinc-950 transition-colors duration-200 group-hover:text-accent">
          {project.title}
          {project.ongoing && (
        ...
        <p className="line-clamp-1 text-sm text-zinc-600">{project.summary}</p>
      </div>

      <div className="font-mono text-xs text-zinc-600 sm:col-span-3">
        {project.stack.join(" · ")}
      </div>
